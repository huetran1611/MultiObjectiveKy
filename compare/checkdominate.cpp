...
bool dominatedinpareto(Individual indi,int bo){
    if(pareto[bo].size()==0)return false;
    for(int i=0;i<pareto[bo].size();i++){
        if(checkdomination(pareto[bo][i],indi))return true;
    }
    return false;
}
...
bool inpareto(Individual indi,int bo){
    for(int i=0;i<pareto[bo].size();i++){
        if(abs(indi.fitness1-pareto[bo][i].fitness1)<1e-4&&abs(indi.fitness2-pareto[bo][i].fitness2)<1e-4) return true;
...
int main(){
    ...
    for(int i=0;i<solss.size();i++){
        ...
        if(dominatedinpareto(indi,bo))dominatedcount++;
        else{
            int cou=0;
            for(int j=0;j<pareto[bo].size();j++){
                if(checkdomination(indi,pareto[bo][j]))cou++;
            }
